...
public class Maze {
	...
	public List<MazeNode> dfsRecursive(int startRow, int startCol, int endRow, int endCol){
		...
		visited.add(start);
		dfsSearchRecursive(start,goal,visited,parents);
		
		return reconfigurePath(start,goal,parents);
		
		
	}
	
	//recursive solution for depth first search.
	private void dfsSearchRecursive(MazeNode start, MazeNode goal, HashSet<MazeNode> visited, HashMap<MazeNode,MazeNode> parents){
		...
		for(MazeNode neighbor : start.getNeighbors()) {
			if(visited.contains(neighbor) == false) {
				visited.add(neighbor);
				parents.put(neighbor,start);
				dfsSearchRecursive(neighbor,goal,visited,parents);
			}
		}
		
	}
	
	private List<MazeNode> reconfigurePath(MazeNode start,MazeNode goal,HashMap<MazeNode,MazeNode> parents){
...
/*
	public List<MazeNode> bfsRefactored(int startRow, int startCol, 
			int endRow, int endCol) {
		// Initialize
		MazeNode start = cells[startRow][startCol];
		MazeNode goal = cells[endRow][endCol];
	...
	private static boolean bfsSearch(MazeNode start, MazeNode goal, 
			HashMap<MazeNode, MazeNode> parentMap) 
	{
		
		HashSet<MazeNode> visited = new HashSet<MazeNode>();
		Queue<MazeNode> toExplore = new Linked<MazeNode>();
		toExplore.add(start);
		boolean found = false;
...
*/
	...
	public static void main(String[] args) {
		...
		List<MazeNode> path = maze.dfsRecursive(3, 3, 2, 0);
		maze.setPath(path);
		System.out.println("\n");
		maze.printMaze();
		
//		maze.clearPath();
//		maze.setPath(maze.dfsRecursive(3, 3, 2, 0));
//		System.out.println("\n");
//		maze.printMaze();
