...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {		
		...
		if (offline) {
		    ...
		    earthquakesURL = "https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/2.5_week.atom";  // The same feed, but saved August 7, 2015
		}
		else {
	...
	private void addKey() {	
		...
		fill(153,102,0);
		triangle(50,110,40,110,45,100);
		
		fill(0);
		textAlign(RIGHT, CENTER);
		textSize(12);
		text("City Marker", 130, 105);
		
		fill(0);
		textAlign(RIGHT, CENTER);
		textSize(12);
		text("Land Quake", 135, 125);
		
		fill(0);
		textAlign(RIGHT, CENTER);
		textSize(12);
		text("Ocean Quake", 143, 145);
		...
		textAlign(RIGHT, CENTER);
		textSize(12);
		text("Shallow", 110, 182);

		textAlign(RIGHT, CENTER);
		textSize(12);
		text("Intermidiate", 138, 200);
		...
		fill(255,255,0);
		ellipse(45,184,10,10);
		
		fill(0,0,255);
		ellipse(45,201,10,10);
		 
		fill(255,0,0);
		ellipse(45,218,10,10);
		
		fill(0);
		textAlign(RIGHT, CENTER);
		textSize(12);
		text("Past Hour", 126, 235);
...
//		text("Below 4.0", 75, 225);
	...
	private void printQuakes() 
	{
		...
		for(Marker cm:countryMarkers) { //need to make sure this line is here to make
			for(Marker m: quakeMarkers) {
				String countryMarker = (String)cm.getProperty("name");
				EarthquakeMarker eq = (EarthquakeMarker)m;
				if(eq.isOnLand) {
					LandQuakeMarker land = (LandQuakeMarker)eq;
					String country = land.getCountry();
					if(country==countryMarker && !map.containsKey(country)) {
						map.put(country,1);
					}else if(country==countryMarker && map.containsKey(country)) {
		...
		int sumOfLandQuakes = 0;
		for(String x:map.keySet()) {
			sumOfLandQuakes += map.get(x); 
			System.out.println(x + " had " + map.get(x) + " earthquakes in their country on land");
		}
		
		int Oceancount = quakeMarkers.size() - sumOfLandQuakes;
		System.out.println("There were " + Oceancount + " number of earthquakes over the ocean");
